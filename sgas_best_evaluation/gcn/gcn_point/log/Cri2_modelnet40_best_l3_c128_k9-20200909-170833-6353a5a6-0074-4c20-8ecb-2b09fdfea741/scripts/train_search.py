...
def main():
    ...
    normal_selected_idxs = torch.tensor(len(model.alphas_normal[:-1]) * [-1], requires_grad=False, dtype=torch.int).cuda()
    normal_candidate_flags = torch.tensor(len(model.alphas_normal[:-1]) * [True], requires_grad=False, dtype=torch.bool).cuda()
    ...
    # print(F.softmax(torch.stack(model.alphas_normal[:-1], dim=0), dim=-1).detach())
    # print(F.softmax(torch.stack(tuple(model.alphas_normal[-1]), dim=0), dim=-1).detach())
    ...
    for epoch in range(args.epochs):
        ...
        logging.info('train_acc %f\tvalid_overall_acc %f \t valid_class_acc %f', train_acc, valid_overall_acc, valid_class_acc)
