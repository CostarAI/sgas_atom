...
class MixedOp(nn.Module):
    ...
    def forward(self, x, edge_index, weights,selected_idx=None,att=None):   ##BUG
        if selected_idx is None:
            return sum(w * op(x, edge_index, att) for w, op in zip(weights, self._ops))
        else: # unchosen operations are pruned
            return self._ops[selected_idx](x, edge_index, att)
        ...
        for i in range(self._steps):
            ...
            for j, h in enumerate(states):
                if selected_idxs[offset + j] == -1: # undecided mix edges
                    o = self._ops[offset + j](h, edge_index, weights[offset + j],att=att)   # call the gcn module,
                elif selected_idxs[offset + j] == PRIMITIVES.index('none'): # pruned edges
                    pass
                else: # decided discrete edges
                    o = self._ops[offset + j](h, edge_index, None ,selected_idxs[offset + j], att=att)
                o_list.append(o)
            s = sum(o_list)
            offset += len(states)
            states.append(s)
...
class Network(nn.Module):
    def __init__(self, C, num_classes, num_cells, criterion,
        ...
        self.normal_selected_idxs = torch.tensor((len(self.alphas_normal)-1) * [-1], requires_grad=False, dtype=torch.int)
        self.normal_candidate_flags = torch.tensor((len(self.alphas_normal)-1) * [True], 
    ...
    def _initialize_alphas(self):
        ...
        for i in range(self._steps):
            for n in range(2 + i):
                self.alphas_normal.append(Variable(1e-3 * torch.randn(num_ops).cuda(), requires_grad=True))
        self.att=Variable(1e-3*torch.randn(3, 5).cuda(), requires_grad=True)
        self.alphas_normal.append(self.att)
        self._arch_parameters = [
            self.alphas_normal,
        ]
