...
###############所有原子操作
class x_i_op_c(nn.Module):
    def __init__(self):
        super(x_i_op_c,self).__init__()
    def forward(self,x_i,x_j):
        return x_i

class x_j_op_c(nn.Module):
    def __init__(self):
        super(x_j_op_c,self).__init__()
    def forward(self,x_i,x_j):
        return x_j

class xij_sub_c(nn.Module):
    def __init__(self):
        super(xij_sub_c,self).__init__()
    def forward(self,x_i,x_j):
        return x_i-x_j

class xce_sub_c(nn.Module):
    def __init__(self):
        super(xce_sub_c,self).__init__()
    def forward(self,x_i,x_j):
        m_i=torch.mean(x_i, dim=-1, keepdim=True)
        return x_i - m_i

class xij_eud_c(nn.Module):
    def __init__(self):
        super(xij_eud_c,self).__init__()
    def forward(self,x_i,x_j):
        return torch.norm(x_i-x_j, dim=1).unsqueeze(1).repeat(1, x_i.shape[1], 1, 1)

###############所有原子操作
class Atom_search(nn.Module):
    def __init__(self, in_channels, out_channels ,act='relu', norm=None, bias=False, negative_slope=0.2):
        super(Atom_search, self).__init__()
        self.i_c = BasicConv([in_channels, out_channels], act, norm, bias)
        self.c_c = BasicConv([2 * out_channels, out_channels], act, norm, bias)
        ...
        xce_sub = xce_sub_c()
        xij_eud = xij_eud_c()
        self.ATOM_OPS = [x_i_op, x_j_op, xij_sub, xce_sub, xij_eud]
    ...
    #     self.att=att
    #     return self
    def forward(self, x, edge_index, att, start, end):
        
        edge_index = add_self_loops(edge_index)
        x_i = batched_index_select(x, edge_index[1])    # B,C,N,K
        x_j = batched_index_select(x, edge_index[0])
        # softmax改为entmax
        all_weights=F.softmax(torch.Tensor(att),dim=1)
        # all_weights=F.softmax(att,dim=1)
        # all_weights=entmax15(att,dim=1)
        # all_weights=sparsemax(att,dim=1)
        x_i=self.i_c(x_i)
        x_j=self.i_c(x_j)
        device=x_i.device
        # fenzhi0
        op0=torch.zeros(x_i.shape).to(device)
        for weight, atom_op in zip(all_weights[0, start:end],self.ATOM_OPS):
            # import pdb;pdb.set_trace()
            op0=op0+atom_op(x_i,x_j)*weight
        #fenzhi1
        op1=torch.zeros(x_i.shape).to(device)
        for weight, atom_op in zip(all_weights[1, start:end],self.ATOM_OPS):
            op1=op1+atom_op(x_i,op0)*weight
        #fenzhi2
        op2=torch.zeros(x_i.shape).to(device)
        for weight, atom_op in zip(all_weights[2, start:end],self.ATOM_OPS):
            op2=op1+atom_op(op0, x_j)*weight
        #concate
        op3=torch.cat((op1,op2),dim=1)
        output=self.c_c(op3)
        output, _ = torch.max(output, dim=-1, keepdim=True)
        return output
        
class Conv1(nn.Module):
    def __init__(self, in_channels, out_channels ,act='relu', norm=None, bias=False, negative_slope=0.2):
        super(Conv1, self).__init__()
        self.atom = Atom_search(in_channels, out_channels ,act='relu', norm=None, bias=False, negative_slope=0.2)
        self.start = 0
        self.end = 5
    def forward(self, x, edge_index, att):
        # import pdb;pdb.set_trace()
        output = self.atom(x, edge_index, att, start=self.start, end=self.end)
        # import pdb;pdb.set_trace()
        return output
