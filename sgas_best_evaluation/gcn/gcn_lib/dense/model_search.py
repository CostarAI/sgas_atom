...
class MixedOp(nn.Module):
    ...
    def forward(self, x, edge_index, weights, att,selected_idx=None):   ##BUG
        ...
        for i in range(self._steps):
            ...
            for j, h in enumerate(states):
                if selected_idxs[offset + j] == -1: # undecided mix edges
                    o = self._ops[offset + j](h, edge_index, weights[offset + j],att)   # call the gcn module,
                elif selected_idxs[offset + j] == PRIMITIVES.index('none'): # pruned edges
                    pass
                else: # decided discrete edges
                    o = self._ops[offset + j](h, edge_index, None, att,selected_idxs[offset + j])
                o_list.append(o)
            s = sum(o_list)
            offset += len(states)
            states.append(s)
